...
class zipfian_int_distribution {
   ...
   public:
    ...
    struct param_type {
        ...
        explicit param_type(_IntType __a, _IntType __b, double __theta, double __zeta)
            : _M_a(__a), _M_b(__b), _M_theta(__theta), _M_zeta(__zeta), _M_zeta2theta(zeta(2, __theta))
        {
        ...
        {
            return __p1._M_a == __p2._M_a && __p1._M_b == __p2._M_b && __p1._M_theta == __p2._M_theta
                   && __p1._M_zeta == __p2._M_zeta && __p1._M_zeta2theta == __p2._M_zeta2theta;
        }
   ...
   public:
    ...
    result_type operator()(_UniformRandomNumberGenerator& __urng, const param_type& __p)
    {
        ...
        double eta =
            (1 - std::pow(2.0 / (__p.b() - __p.a() + 1), 1 - __p.theta())) / (1 - __p.zeta2theta() / __p.zeta());
...
int Zipf(int min, int max, double theta)
{
    ...
    static zipfian_int_distribution<int> distribution(min, max, theta);
    return distribution(generator);
}

uint64_t NextValue(void* self)
{
    ZipfGen* zipfGen = (ZipfGen*)self;
    return static_cast<zipfian_int_distribution<int>*>(zipfGen->data.distribution_)
        ->
        operator()(*static_cast<std::default_random_engine*>(zipfGen->data.generator_));
}

static ZipfGenIntf g_ZipfGenIntf = {
    nextValue : NextValue,
};

void GenerateZipfGen(void** zipf, ZipfGenConfigure* conf)
{
    ZipfGen* zipfGen = new ZipfGen;
    assert(zipfGen != nullptr);
    zipfGen->data.generator_ = new std::default_random_engine();
    assert(zipfGen->data.generator_ != nullptr);
    zipfGen->data.distribution_ = new zipfian_int_distribution<int>(conf->min, conf->max, conf->theta);
    assert(zipfGen->data.distribution_ != nullptr);
    zipfGen->intf = &g_ZipfGenIntf;
    *zipf = (void*)zipfGen;
}

void DestroyZipfGen(void** zipf)
{
    ZipfGen* zipfGen = *(ZipfGen**)zipf;
    delete static_cast<std::default_random_engine*>(zipfGen->data.generator_);
    delete static_cast<zipfian_int_distribution<int>*>(zipfGen->data.distribution_);
    delete zipfGen;
    *zipf = NULL;
}